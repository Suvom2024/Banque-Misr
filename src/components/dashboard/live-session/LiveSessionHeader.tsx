    ...
    <header className="bg-white/90 backdrop-blur-md sticky top-0 z-10 border-b border-bm-grey/80 shadow-sm flex-shrink-0">
      <div className="w-full px-4 lg:px-6">
        <div className="flex items-center justify-between h-16">
          <div className="flex flex-col justify-center flex-shrink-0 min-w-0 max-w-[50%]">
            <Link
              href="/training-hub"
            ...
            <div className="flex items-center gap-2 lg:gap-3">
              {showLiveBadge && (
                <span className="inline-flex items-center rounded-md bg-red-50 px-1.5 py-0.5 text-[10px] lg:text-xs font-medium text-red-700 ring-1 ring-inset ring-red-600/10 animate-pulse flex-shrink-0">
                  LIVE
                </span>
              )}
              {showLiveBadge && <span className="text-[10px] lg:text-xs text-bm-text-secondary hidden sm:inline">Scenario #241</span>}
              <h1 className="text-base lg:text-lg font-bold text-bm-maroon tracking-tight leading-tight truncate">{scenarioTitle}</h1>
            </div>
          </div>
