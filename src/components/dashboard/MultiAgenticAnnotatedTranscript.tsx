...
export function MultiAgenticAnnotatedTranscript({ entries }: MultiAgenticAnnotatedTranscriptProps) {
  ...
  return (
    <div className="bg-bm-white rounded-2xl shadow-card border border-bm-grey flex flex-col h-[600px]">
      <div className="p-4 border-b border-bm-grey bg-bm-light-grey/30 rounded-t-2xl">
        <h3 className="font-bold text-bm-text-primary flex items-center gap-2">
          <span className="material-symbols-outlined text-bm-text-secondary">description</span>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-4">
        {entries.map((entry) => (
          <div key={entry.id} className="flex flex-col gap-1 items-end group">
            {entry.speaker === 'user' && (
              <>
                <div className="bg-bm-maroon text-bm-white p-3 rounded-2xl rounded-tr-none text-sm max-w-[90%] shadow-sm relative">
                  ...
                  {hoveredId === entry.id && entry.highlights && entry.highlights[0].tooltip && (
                    <div className="hidden group-hover:block absolute right-0 top-full mt-2 w-64 bg-white p-3 rounded-lg shadow-xl border border-bm-gold z-10 text-bm-text-primary animate-[fadeIn_0.2s_ease-out]">
                      <div className="flex items-start gap-2">
                        <span className="material-symbols-outlined text-bm-gold text-sm">lightbulb</span>
                        <div>
                          <p className="text-xs font-bold text-bm-gold-dark uppercase mb-1">Empathy Hint</p>
                          <p className="text-xs text-bm-text-secondary">{entry.highlights[0].tooltip}</p>
                        </div>
                      </div>
                    </div>
                  )}
                  {entry.complianceAlert && (
                    <div className="absolute -right-2 -top-2 bg-white rounded-full p-1 shadow-md border border-bm-maroon cursor-pointer group-hover:scale-110 transition-transform">
                      <span className="material-symbols-outlined text-bm-maroon text-sm block">gavel</span>
                    </div>
                  )}
                </div>
                <div className="text-[10px] text-bm-text-subtle font-medium">You • {entry.timestamp}</div>
              </>
            )}
            {entry.speaker === 'client' && (
              <>
                <div className="bg-bm-light-grey text-bm-text-primary p-3 rounded-2xl rounded-tl-none text-sm max-w-[90%] border border-bm-grey shadow-sm">
                  <p>{entry.message}</p>
                </div>
                <div className="text-[10px] text-bm-text-subtle font-medium">Client • {entry.timestamp}</div>
              </>
            )}
            {entry.complianceAlert && (
              <div className="flex justify-center my-2 w-full">
                <div className="bg-red-50 border border-red-100 px-3 py-1.5 rounded-full flex items-center gap-2">
                  <span className="material-symbols-outlined text-bm-maroon text-xs">warning</span>
                  <span className="text-[10px] font-bold text-bm-maroon uppercase tracking-wide">{entry.complianceAlert}</span>
                </div>
              </div>
            )}
          </div>
