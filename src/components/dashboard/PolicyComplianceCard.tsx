...
export function PolicyComplianceCard({
  ...
  return (
    <div className="bg-bm-white rounded-xl shadow-card border border-bm-grey/60 overflow-hidden hover:shadow-card-hover transition-all">
      <div className="border-l-[6px] border-l-bm-maroon p-6">
        <div className="flex justify-between items-start mb-4">
          <div className="flex items-center gap-3">
            <div className="bg-red-50 p-2 rounded-lg text-bm-maroon">
              <span className="material-symbols-outlined">gavel</span>
            </div>
            <div>
              <h4 className="font-bold text-bm-text-primary">Policy Compliance</h4>
              <p className="text-xs text-bm-text-secondary">Regulatory & Script Adherence</p>
            </div>
          </div>
          <span className="inline-flex items-center rounded-md bg-red-50 px-2 py-1 text-xs font-medium text-bm-maroon ring-1 ring-inset ring-red-600/20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="flex flex-col justify-end">
            <h5 className="text-xs font-bold text-bm-text-subtle uppercase mb-2">Script Adherence Rate</h5>
            <div className="bg-bm-light-grey rounded-lg p-4">
              {adherenceMetrics.map((metric, index) => {
                const percentage = (metric.current / metric.total) * 100
                return (
                  <div key={index} className={index > 0 ? 'mt-4' : ''}>
                    <div className="flex justify-between text-xs font-bold mb-1 text-bm-text-primary">
